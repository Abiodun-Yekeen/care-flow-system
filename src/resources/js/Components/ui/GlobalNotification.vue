...
<!--<template>-->
<!--    <div class="w-full z-40 flex flex-col gap-2 pointer-events-none sticky top-0 bg-white backdrop-blur-sm">-->
<template>
    <div class="flex flex-col gap-2 pointer-events-none">
        <div v-for="alert in notificationStore.criticalAlerts" :key="alert.id" class="pointer-events-auto">
            <div :class="['mx-4 mt-2 flex items-center p-3 rounded shadow-lg text-white font-medium text-sm border-l-4 transition-all', getClasses(alert.type)]">
                ...
                <div class="flex-1">
                    <span class="font-bold opacity-80 mr-2 uppercase text-[10px] tracking-widest text-white/90">Critical:</span>
                    {{ alert.message }}
                </div>
                <button @click="notificationStore.removeCriticalAlert(alert.id)" class="ml-4 hover:bg-black/10 rounded-full p-1 transition-colors text-xl">&times;</button>
            </div>
        </div>
        ...
        >
            <div v-if="notificationStore.current.visible" class="pointer-events-auto">
                <div :class="['mx-4 my-2 flex items-center p-3 rounded shadow-md text-white font-medium text-sm border-l-4', getClasses(notificationStore.current.type)]">
                    <div class="mr-3 flex-shrink-0">
                    ...
                    <button @click="notificationStore.hide" class="ml-4 hover:bg-black/10 rounded-full p-1 transition-colors text-xl">&times;</button>
                </div>
            </div>
        </transition>

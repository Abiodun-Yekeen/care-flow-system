...
<template>
    <ContextModuleLayout>
        ...
        <div class="min-h-screen lg:h-full flex flex-col bg-gray-50 overflow-y-auto lg:overflow-hidden">
            <PageHeader title="My Desk" subtitle="Process incoming correspondence" />

            <!-- Main Container: Stack on mobile (col), Row on desktop -->
            <div class="flex flex-col lg:flex-row flex-1 overflow-hidden p-2 sm:p-4 gap-4">

                <!-- 1. INBOX COLUMN -->
                <!-- Hidden on mobile if a file is selected to give space to details -->
                <div
                    :class="[
                        'w-full lg:w-80 flex flex-col bg-white rounded-xl shadow-sm border overflow-hidden shrink-0 transition-all',
                        selectedFile ? 'hidden lg:flex' : 'flex h-[500px] lg:h-auto'
                    ]"
                >
                    ...
                    <!-- Fixed height scroll area for Inbox -->
                    <div class="flex-1 overflow-y-auto custom-scrollbar">
                        <div v-for="file in filteredFiles" :key="file.id"
                             @click="selectedFile = file"
                             :class="[
                                'p-4 border-b cursor-pointer transition-all relative group',
                                selectedFile?.id === file.id ? 'bg-blue-50/50' : (file.status === 'pending' || isNew(file.date_received) ? 'bg-amber-50/40' : 'hover:bg-gray-50')
                             ]">
                            <div v-if="selectedFile?.id === file.id" class="absolute left-0 top-0 bottom-0 w-1 bg-blue-600 z-10"></div>
                            <h4 class="text-xs font-bold text-gray-800 line-clamp-2 leading-snug">{{ file.source_name }}</h4>
                            <p class="text-[10px] text-gray-500">{{ file.file_number }} - {{ file.date_received }}</p>
                        </div>
                    </div>
                    ...
                    <div v-if="props.files.links.length > 3" class="p-3 border-t bg-gray-50 flex justify-between items-center">
                        ...
                        <span class="text-[10px] font-bold text-gray-400">{{ props.files.current_page }} / {{ props.files.last_page }}</span>
                ...
                <div
                    :class="[
                        'flex-1 flex flex-col bg-white rounded-xl shadow-sm border overflow-hidden transition-all',
                        !selectedFile ? 'hidden lg:flex' : 'flex'
                    ]"
                >
                    <template v-if="selectedFile">
                        <!-- Mobile Back Button -->
                        <div class="lg:hidden p-2 bg-gray-100 border-b">
                            <button @click="selectedFile = null" class="flex items-center text-blue-600 text-xs font-bold">
                                <ChevronRight class="rotate-180" :size="16" /> Back to Inbox
                            </button>
                        </div>

                        <div class="p-4 border-b bg-gray-50/30">
                            <div class="flex flex-col sm:flex-row justify-between items-start gap-2">
                                <h2 class="text-xl sm:text-3xl font-black text-blue-900 tracking-tighter break-all">
                                    {{ selectedFile.file_number }}
                                </h2>
                                <div class="flex flex-row lg:flex-col items-center lg:items-end gap-2">
                                    <span class="px-3 py-1 bg-gray-200 text-gray-700 rounded-full text-[10px] font-bold uppercase">{{ selectedFile.status }}</span>
                        ...
                        <div class="flex-1 overflow-y-auto p-4 sm:p-6">
                            ...
                            <div class="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
                                <div class="flex items-center gap-2 mb-4 text-slate-800 font-bold">
                                    <FileText :size="18" class="text-slate-400" /> File Details
                                </div>
                                <div class="space-y-3 text-sm">
                                    <div class="flex flex-col sm:flex-row sm:justify-between border-b pb-1">
                                        <span class="text-slate-500">Subject:</span>
                                        <span class="font-medium">{{ selectedFile.subject }}</span>
                                    </div>
                                    <!-- Repeat for other rows -->
                                    <div class="flex flex-col sm:flex-row sm:justify-between border-b pb-1">
                                        <span class="text-slate-500">Current Office:</span>
                                        <span class="font-medium">{{ selectedFile.department }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <!-- Attachments -->
                            <div class="mt-6">
                                <h3 class="text-[11px] font-black text-gray-400 uppercase tracking-widest border-b pb-2 mb-4">Attached Files</h3>
                                <div v-if="selectedFile.attachments?.length > 0" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <FormFilePreviewCard
                                ...
                                <div v-else class="p-8 text-center bg-gray-50 rounded-xl border-2 border-dashed">
                            ...
                            <!-- Timeline -->
                            <div class="mt-8">
                                <h3 class="text-[11px] font-black text-gray-400 uppercase tracking-widest border-b pb-2 mb-4">Timeline Summary</h3>
                                <Timeline :movements="selectedFile.movements || []" />
                            </div>
                        </div>
                    </template>

                    <div v-else class="flex-1 flex flex-col items-center justify-center text-gray-300 p-12 text-center">
                        <Inbox :size="48" class="opacity-10 mb-2" />
                        <p class="text-xs italic font-medium">Select a file from the inbox to begin processing</p>
                    </div>
                </div>
                ...
                <div
                    :class="[
                        'w-full lg:w-96 flex flex-col bg-white rounded-xl shadow-sm border overflow-hidden shrink-0 transition-all',
                        !selectedFile ? 'hidden lg:flex' : 'flex'
                    ]"
                >
                    <div v-if="selectedFile" class="flex flex-col h-full">
                        ...
                        <div class="flex-1 overflow-y-auto p-5">
                            <!-- Forms go here (Minute, Note, etc) -->
                            <div v-if="rightTab === 'action'" class="space-y-4">
                                <FormTextarea id="minute" v-model="form.minute" label="Minute" required />
                                <FormTextarea id="remark" v-model="form.remark" label="Note" />
                                <LoadButton @click="submit_treat_file" :loading="form.processing">Treated</LoadButton>
                            </div>

                            <div v-if="rightTab === 'history'" class="space-y-4">
                                <FormSelect v-model="routeForm.department" label="Department" :options="metaStore.department" />
                                <FormSelect v-model="routeForm.staff" label="Staff" :options="staff" :disabled="!routeForm.department" />
                                <FormFileUpload v-model="routeForm.scanned_file" />
                                <LoadButton @click="submit_route_file" :loading="routeForm.processing">Route File</LoadButton>
                            </div>
                        </div>
                    </div>
                    <div v-else class="h-full flex items-center justify-center text-gray-300 text-xs italic">
                        Select a file to act
                    </div>
                </div>
...
<style scoped>
/* Improved custom scrollbar for better touch/mouse balance */
...
/* Ensure nice scrolling on mobile devices */
